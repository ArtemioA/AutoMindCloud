...
// Unified mesh loader for both base64 data URLs and external files
export function loadMesh(filename, material, onLoad, onError) {
    if (
        filename.startsWith('data:model/stl;base64,') ||
    ...
    parse(content) {
        ...
        function processMaterial(node) {
            ...
            matNodes.forEach(n => {
                ...
                if (type === 'color') {
                    const rgba =
                        n.getAttribute('rgba').split(/\s/g).map(v => parseFloat(v));
